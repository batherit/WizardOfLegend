	...
	if ((m_fElapsedTime += _fDeltaTime) > m_fDistAdjustmentTime) {
		...
		if (m_rOwner.GetListMonsters().size() > 0) {
			if (rand() % 10 < 7) {
				...
				else if (iRandAttack == 1) m_rOwner.GetStateMgr()->SetNextState(new CMiddleBossState_IceCrystal(m_rOwner));
			...
			if(rand() % 10 < 7) m_rOwner.GetStateMgr()->SetNextState(new CMiddleBossState_Summon(m_rOwner));
			else {
				int iRandAttack = rand() % 3;
				if (iRandAttack == 0) m_rOwner.GetStateMgr()->SetNextState(new CMiddleBossState_Fire(m_rOwner));
				else if (iRandAttack == 1) m_rOwner.GetStateMgr()->SetNextState(new CMiddleBossState_IceCrystal(m_rOwner));
